...
class ShoppingCart:
    ...
    def cart_contents(self):
        ...
        for name, qty, price in self.items:
            ...
            print(f"Total after discount : Ksh {self.calculate_total():.2f}\n")
...
class DicountedCart(ShoppingCart):
    ...
    def cart_contents(self):
        print("Cart Contents (Discounted): ")
        for name, qty, price in self.items:
            print(f"  {name}:- {qty} @ Ksh {price:.2f} each")
        initial_total = super().calculate_total()
        discount = initial_total * self.discount_rate
        final_total = self.calculate_total()
        print(f"Subtotal: Ksh {initial_total:.2f}")
        print(f"Discount ({self.discount_rate * 100:.0f}%): -Ksh {discount:.2f}")
        print(f"Total after discount: Ksh {final_total:.2f}\n")
...
class TaxedCart(ShoppingCart):
    ...
    def cart_contents(self):
        ...
        final_total = self.calculate_total()
        print(f"Subtotal: Ksh {initial_total:.2f}")
        print(f"Tax ({self.tax_rate * 100:.0f}%): +Ksh {tax:.2f}")
        print(f"Total after tax: Ksh {final_total:.2f}\n")
...
#class - ShoppingCart, DiscountedCart, TaxedCart
#inheritance - class DiscountedCart(ShoppingCart)
#method override - (calculate_total() in subclass)
#polymorphism - checkout (cart) works on any cart
#